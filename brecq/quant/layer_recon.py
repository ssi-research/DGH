...
def layer_reconstruction(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
    ...
    if opt_mode != 'mse':
        ...
        cached_grads = None
    device = 'cuda'
    inds = torch.randperm(cached_inps.size(0))
    batch_ind = 0
    for i in range(iters):
        # idx = torch.randperm(cached_inps.size(0))[:batch_size]
        if (batch_ind + 1) * batch_size > inds.shape[0]:
            inds = torch.randperm(cached_inps.size(0))
            batch_ind = 0
        idx = inds[batch_ind * batch_size: (batch_ind + 1) * batch_size]
        batch_ind += 1
