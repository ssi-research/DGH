...
class UniformAffineQuantizer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, eps=1e-8):
        ...
        # start quantization
        x_int = round_ste(x / (self.delta+eps)) + self.zero_point
        x_quant = torch.clamp(x_int, 0, self.n_levels - 1)
        x_dequant = (x_quant - self.zero_point) * self.delta
        return x_dequant

    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False, eps=1e-8):
            ...
            if 'max' in self.scale_method:
                ...
                for i in range(80):
                    ...
                    if score < best_score:
                        ...
                        zero_point = (- new_min / (delta + eps)).round()
            else:
                raise NotImplementedError

        return delta, zero_point

    def quantize(self, x, max, min, eps=1e-8):
        delta = (max - min) / (2 ** self.n_bits - 1)
        zero_point = (- min / (delta + eps)).round()
