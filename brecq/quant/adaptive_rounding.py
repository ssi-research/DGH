...
class AdaRoundQuantizer(nn.Module):
    ...
    def forward(self, x, eps=1e-8):
        if self.round_mode == 'nearest':
            x_int = torch.round(x / (self.delta + eps))
        elif self.round_mode == 'nearest_ste':
            x_int = round_ste(x / (self.delta + eps))
        elif self.round_mode == 'stochastic':
            x_floor = torch.floor(x / (self.delta + eps))
            rest = (x / (self.delta + eps)) - x_floor  # rest of rounding
    ...
    def init_alpha(self, x: torch.Tensor, eps=1e-8):
        ...
        if self.round_mode == 'learned_hard_sigmoid':
            ...
            rest = (x / (self.delta + eps)) - x_floor  # rest of rounding [0, 1)
